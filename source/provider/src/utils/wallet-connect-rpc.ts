...
class WalletConnectRPC implements SimplifiedRPC {
  ...
  public call(
    handler: string,
    args: any[],
    options: { timeout: number } = { timeout: DEFAULT_TIMEOUT }
  ): Promise<any> {
    const timeout =
    ...
    return new Promise((resolve, reject) => {
      ...
      const rejectAndClear = (error: any) => {
        if (timeoutFun) clearTimeout(timeoutFun);
        reject(error);
      };
  ...
  private async requestReadState(
    args: any[],
    resolve: (value: any) => void,
    reject: (reason?: any) => void
  ): Promise<void> {
  ...
  private async clearClient(): Promise<void> {
    this.wcClient.off("disconnect");
  ...
  private isAllWhiteListed(newWhiteList: string[]): boolean {
    return newWhiteList.every((element) => this.whitelist.includes(element));
  }

  private addToWhiteList(newWhiteList: string[]): void {
